...
class UsbPrinterConnector implements PrinterConnector<UsbPrinterInput> {
  ...
  Future<bool> _close() async {
    if (Platform.isWindows) {
      final result = await flutterPrinterChannel.invokeMethod('close');
      return _nativeBool(result);
    }
    return false;
  }

  /// Win32 plugin may return `true` (bool) or `1` (int) depending on codec.
  static bool _nativeBool(dynamic value) => value == true || value == 1;
  ...
  Future<bool> send(List<int> bytes) async {
    if (Platform.isAndroid)
      ...
      try {
        ...
        final result =
            await flutterPrinterChannel.invokeMethod('printBytes', params);
        return _nativeBool(result);
      } catch (e) {
        await this._close();
        return false;
