...
{
    if (_hPrinter != INVALID_HANDLE_VALUE && _hPrinter != NULL)
    {
...
BOOL PrintManager::printBytes(std::vector<uint8_t> data)
{
    ...
    if (_hPrinter == INVALID_HANDLE_VALUE || _hPrinter == NULL)
    {
        throw std::exception("Printer handle is invalid. Call connectPrinter first.");
    }
...
BOOL PrintManager::close()
{
    if (_hPrinter != INVALID_HANDLE_VALUE && _hPrinter != NULL)
    {
        const BOOL closed = ClosePrinter(_hPrinter);
        _hPrinter = INVALID_HANDLE_VALUE;
        return closed;
    }
